...
class tranceblackman_comSpider(Spider):
    name = "tranceblackman_com"
    count = 0
    custom_output = False
    use_selenium = True
    start_url = [
            'https://tranceblackman.com/journal/'
    ]
    ...
    def parse1(self, response):
        urls = response.xpath('//div[@id="loop-container"]//article//h2/a')
        if not urls:
            return
    ...
    def parse(self, response):
        ...
        item = OrderedDict()
        keys_texts = response.xpath('//div[@class="post-tags"]/ul/li/a/text()').extract()
        keywords = []
        ...
        img_urls = response.xpath('//div[@class="featured-image"]/img')
        ...
        item['images'] = images
        item['themes'] = [{"Id": 0, "ArticleId": 0, "Theme": "Canada"}]
