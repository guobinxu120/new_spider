...
class theskinnyscout_comSpider(Spider):
    name = "theskinnyscout_com"
    count = 0
    custom_output = False
    use_selenium = True
    start_url = [
            'https://www.theskinnyscout.com/category/beauty/',
            'https://www.theskinnyscout.com/category/fashion/',
            'https://www.theskinnyscout.com/category/lifestyle-health/',
            'https://www.theskinnyscout.com/category/home-parenting/',
            'https://www.theskinnyscout.com/category/travel/'
    ]
    ...
    def parse1(self, response):
        urls = response.xpath('//ul[@class="penci-grid"]/li//h2[@class="grid-title"]/a/@href').extract()
        if not urls:
            return
    ...
    def parse(self, response):
        ...
        contributor = response.xpath('//span[@class="author vcard"]/span/text()').extract_first()
        if contributor:
        ...
        img_urls = response.xpath('//div[@class="inner-post-entry"]//img') + response.xpath('//div[@class="post-image"]//img')
        ...
        for img_tag in img_urls:
            ...
            if not img_url:
                continue
            if ('.svg' in img_url) or ('.gif' in img_url) or ('theskinnyscout.com' not in img_url):
        ...
        item['images'] = images
        item['themes'] = [{"Id": 0, "ArticleId": 0, "Theme": "Singapore"}]
