...
class careeralley_comSpider(Spider):
    name = "careeralley_com"
    count = 0
    custom_output = False
    use_selenium = True
    start_url = [
    ...
    def start_requests(self):
        for url in self.start_url:
            payload = {'action': 'be_ajax_load_more',
                       'page': '1',
                       'query[category_name]': url}
            files = [

            ]
            headers = {
                'Cookie': '__cfduid=d001411849e18c4688bb10d2e21a2b53d1602640886'
            }

            yield FormRequest('https://careeralley.com/wp-admin/admin-ajax.php', self.parse1, formdata=payload,meta={'page': 1, 'category': url})
    ...
    def parse1(self, response):
        ...
        yield FormRequest('https://careeralley.com/wp-admin/admin-ajax.php', self.parse1, formdata=payload,meta=response.meta)
    ...
    def parse(self, response):
        ...
        title = response.xpath('//h1[@itemprop="headline"]/text()').extract_first().strip()
        # if not title:
        ...
        date = response.xpath('//meta[@property="article:published_time"]/@content').extract_first()[:19]
        if date:
        ...
        img_urls = response.xpath('//div[@itemtype="http://schema.org/BlogPosting"]/img') + response.xpath('//span[@itemprop="articleBody"]/p/img')
        ...
        tags = response.xpath('//span[@itemprop="articleBody"]/*')
        total_text = ''
