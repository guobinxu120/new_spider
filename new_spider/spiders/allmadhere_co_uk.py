...
class allmadhere_co_ukSpider(Spider):
    name = "allmadhere_co_uk"
    count = 0
    custom_output = False
    use_selenium = True
    start_url = [
            'https://www.allmadhere.co.uk/'
    ]
    ...
    def parse1(self, response):
        urls = response.xpath('//div[@id="content"]/article//div[@class="floated_summary_left"]/a')
        if not urls:
            return
        ...
        for url_tag in urls:
            ...
            img_url = url_tag.xpath('.//img/@data-p3-pin-img-src').extract_first()
            img_title = url_tag.xpath('.//img/@data-p3-pin-title').extract_first()
            yield Request(url, self.parse, meta={'page': 1, 'title': title, 'img_url': img_url, 'img_title': img_title})

            # break

        response.meta['page'] += 1
        next_page = response.xpath('//nav[@id="nav-below"]/div[contains(@class, "nav-previous")]/a/@href').extract_first()
        if next_page:
    ...
    def parse(self, response):
        ...
        img_urls = response.xpath('//div[@itemprop="articleBody"]//p//img')
        ...
        if not images:
            images.append({"src": response.meta['img_url'], "caption": response.meta['img_title']})

        tags = response.xpath('//div[@itemprop="articleBody"]/*')
        total_text = ''
        for i, tag in enumerate(tags):
            images_tag = tag.xpath('.//img')
            tag_name = tag.root.tag
            if tag_name != 'div' and tag_name != 'figure' and tag_name != 'img' and tag_name != 'script':
