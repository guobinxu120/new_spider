...
class sistyisthenew50_comSpider(Spider):
    name = "sixtyisthenew50_com"
    count = 0
    custom_output = False
    use_selenium = True
    start_url = [
            'https://60isthenew50.com/blog/'
    ]
    ...
    def parse1(self, response):
        urls = response.xpath('//li[@class="edgtf-blog-list-item clearfix"]/div/div/a/@href').extract()
        if not urls:
            return
    ...
    def parse(self, response):
        ...
        title = response.xpath('//div[@class="edgtf-post-text-inner clearfix"]/h2/text()').extract_first().strip()
        # if not title:
        ...
        img_urls = response.xpath('//div[@class="edgtf-post-image"]/img') + response.xpath('//div[@class="wpb_column vc_column_container vc_col-sm-10"]//img')
        ...
        for img_tag in img_urls:
            ...
            if not img_url:
                continue
            if ('.svg' in img_url) or ('.gif' in img_url) or ('60isthenew50.com' not in img_url):
        ...
        tags = response.xpath('//div[@class="wpb_text_column wpb_content_element "]/div/*')
        total_text = ''
