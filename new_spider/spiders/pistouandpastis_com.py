...
class pistouandpastis_comSpider(Spider):
    name = "pistouandpastis_com"
    count = 0
    custom_output = False
    use_selenium = True
    start_url = [
            'https://pistouandpastis.com/2020/09/five-classic-french-stews/'
    ]
    ...
    def parse(self, response):
        post_id = response.xpath('//main[@id="genesis-content"]/article/@class').extract_first().split(' ')[0].split('-')[-1]
        ...
        contributor = response.xpath('//span[@class="entry-author"]/a/span/text()').extract_first()
        if contributor:
        ...
        for i, tag in enumerate(tags):
            ...
            # else:
                ...
                text = delettags(text)
                if text == '<p></p>' or text == '<div></div>' or 'heateor_sss_sharing_container heateor_sss_horizontal_sharing' in text:
                    continue
                total_text += text
        ...
        next_url = response.xpath('//a[@rel="prev"]/@href').extract_first()
        if next_url:
