...
class mommygreenest_comSpider(Spider):
    name = "mommygreenest_com"
    count = 0
    custom_output = False
    use_selenium = True
    start_url = [
            'https://mommygreenest.com/',
    ]
    ...
    def parse1(self, response):
        urls = response.xpath('//div[@id="content"]/div/h2/a/@href').extract()
        if not urls:
            return
        for url in urls:
            # url = 'https://mommygreenest.com/facial-oil/'
            yield Request(url, self.parse, meta={'page': 1, 'origin_url': url})
            # break

        next_url = response.xpath('//div[@class="pagination-next alignright"]/a/@href').extract_first()
        if next_url:
            yield Request(next_url, self.parse1, meta=response.meta)

    def parse(self, response):
        post_id = response.xpath('//div[@id="content"]/div/@class').extract_first()[5:].split(' ')[0]
        ...
        item = OrderedDict()
        keys_texts = response.xpath('//a[@rel="tag"]/text()').extract()
        keywords = []
        for key in keys_texts:
            if len(key) > 50: continue
            keywords.append({'keyword': key.strip()})
        item['keywords'] = keywords
        title = response.xpath('//div[@id="content"]/div/h1/text()').extract_first()
        # if not title:
        ...
        for i, tag in enumerate(tags):
            ...
            # else:
                ...
                text = delettags(text)
                if text == '<p></p>' or text == '<div></div>' or "Like this? You'll love these!" in text \
                        or 'has-inline-color has-white-color' in text or 'social_icons vertical_sharing clearfix' in text \
