...
class careertreasure_comSpider(Spider):
    name = "careertreasure_com"
    count = 0
    custom_output = False
    use_selenium = True
    start_url = [
            'https://careertreasure.com/blog/'
    ]
    ...
    def parse1(self, response):
        urls = response.xpath('//header[@class="entry-header"]/h2/a/@href').extract()
        if not urls:
            return
