...
class mcdaccginc_comSpider(Spider):
    name = "mcdaccginc_com"
    count = 0
    custom_output = False
    use_selenium = True
    start_url = [
            'https://mcdaccginc.com/2020/12/08/virtual-holiday-party-ideas/',
    ]
    ...
    def parse(self, response):
        post_id = response.xpath('//main[@id="main"]/article/@id').extract_first().split('-')[-1]
        ...
        for img_tag in img_urls:
            ...
            if ('mcdaccginc.com/' not in img_url):
                continue
        ...
        for i, tag in enumerate(tags):
            ...
            # else:
                ...
                if text == '<p></p>' or text == '<div></div>' or 'id="amzn-native-ad' in text \
                        or 'id="amzn_assoc_ad_div' in text or 'sharedaddy sd-sharing-enabled' in text \
                        or 'sharedaddy sd-block sd-like' in text or 'content-column one_third' in text \
