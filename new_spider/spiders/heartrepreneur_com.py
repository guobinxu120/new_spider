...
class heartrepreneur_comSpider(Spider):
    name = "heartrepreneur_com"
    count = 0
    custom_output = False
    use_selenium = True
    start_url = [
            'https://heartrepreneur.com/blog/',
    ]
    ...
    def parse1(self, response):
        urls = response.xpath('//article//span[@class="bt_bb_headline_content"]/span/a/@href').extract()
        if not urls:
            return
        for url in urls:
            # url = 'https://heartrepreneur.com/10-tips-on-how-to-make-your-presentation-stand-out/'
            yield Request(url, self.parse, meta={'page': 1, 'origin_url': url})
            # break

        next_url = response.xpath('//p[@class="pageNext"]/a/@href').extract_first()
        if next_url:
            yield Request(next_url, self.parse1, meta=response.meta)

    def parse(self, response):
        post_id = response.xpath('//article[@class]/@class').re(r'[\d.]+')
        if post_id[0] in self.total_post_ids:
            return
        self.total_post_ids.append(post_id[0])
        # java_script = response.xpath('//script[contains(text(), "var rtb_sync = ")]/text()').extract_first()
        item = OrderedDict()
        keys_texts = response.xpath('//div[@class="btTags"]/ul/li/a/text()').extract()
        keywords = []
        for key in keys_texts:
            if len(key) > 50: continue
            keywords.append({'keyword': key.strip()})
        item['keywords'] = keywords
        title = response.xpath('//span[@class="bt_bb_headline_content"]/span/text()').extract_first()
        # if not title:
        #     title = response.xpath('//td[@class="f2"]/text()').extract_first()
        item['title'] = title.strip()
        item['url'] = response.url
        contributor = response.xpath('//div[@class="btAboutAuthor"]//h4/text()').extract_first()
        if contributor:
        ...
        main_img_tag = response.xpath('//div[@class="btArticleMedia"]//img')
        img_urls = response.xpath('//div[@class="btArticleContent "]//div[@class="bt_bb_wrapper"]//img[@class!="thumbnail-image"]')
        ...
        tags = response.xpath('//div[@class="btArticleContent "]//div[@class="bt_bb_wrapper"]/*')
        total_text = ''
        for i, tag in enumerate(tags):
            ...
            # else:
                ...
                #         or 'You’re welcome.' in text or 'Join Our ' in text \
                #         or 'Important Links:' in text or 'content-column one_third' in text \
                #         or '<iframe' in text or 'If you enjoy our recipes you can help support us by' in text \
