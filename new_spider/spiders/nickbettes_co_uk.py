...
class nickbettes_co_ukSpider(Spider):
    name = "nickbettes_co_uk"
    count = 0
    custom_output = False
    use_selenium = True
    start_url = [
            'https://www.nickbettes.co.uk/category/business-coaching-blog/',
    ]
    ...
    def parse1(self, response):
        urls = response.xpath('//div[@class="col-md-12 content"]/h2/a/@href').extract()
        if not urls:
            return
        for url in urls:
    ...
    def parse(self, response):
        post_id = response.xpath('//body[@class]/@class').re(r'[\d.]+')
        ...
        item = OrderedDict()
        keys_texts = response.xpath('//footer//span[@class="cat-links"]/a/text()').extract()
        keywords = []
        for key in keys_texts:
            if len(key) > 50: continue
            keywords.append({'keyword': key.strip()})
        item['keywords'] = keywords
        title = response.xpath('//div[@class="col-md-12 content"]/h2/text()').extract_first()
        # if not title:
        ...
        img_urls = response.xpath('//div[@class="col-md-12 content"]//img')
        ...
        for img_tag in img_urls:
            ...
            if ('nickbettes.co.uk/' not in img_url):
                continue

            if (not img_url) or ('.svg' in img_url) or ('.gif' in img_url):
        ...
        tags = response.xpath('//div[@class="col-md-12 content"]/*')
        total_text = ''
        for i, tag in enumerate(tags):
            ...
            # else:
                ...
                text = delettags(text)
                if text == '<p></p>' or text == '<div></div>' or 'id="comments' in text \
                        or 'class="fsb-clear' in text or 'fsb-social-bar' in text \
                        or 'col-md4 pull-right sidebar-right' in text or 'content-column one_third' in text \
        ...
        prev_url = response.xpath('//div[@class="nav-previous"]/a[@rel="prev"]/@href').extract_first()
        if prev_url:
            yield Request(prev_url, self.parse, meta={'page': 1, 'origin_url': prev_url})
