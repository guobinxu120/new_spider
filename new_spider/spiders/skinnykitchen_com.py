...
class skinnykitchen_comSpider(Spider):
    name = "skinnykitchen_com"
    count = 0
    custom_output = False
    use_selenium = True
    start_url = [
            'https://www.skinnykitchen.com/recipe-index/'
    ]
    ...
    def parse(self, response):
        urls = response.xpath('//ul[@class="recipe-categories grid-parent"]/li/a/@href').extract()
        if not urls:
            return
        for url in urls:
            # url = 'http://balanceguytraining.com/2021/01/01/get-back-in-the-saddle/'
            yield Request(url, self.parse1, meta={'page': 1, 'cat_url': url})
            # break
    ...
    def parse1(self, response):
        ...
        yield Request(cat_url + 'page/' + str(response.meta['page']), self.parse1, meta=response.meta)
