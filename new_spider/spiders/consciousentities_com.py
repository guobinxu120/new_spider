...
class consciousentities_comSpider(Spider):
    name = "consciousentities_com"
    count = 0
    custom_output = False
    use_selenium = True
    start_url = [
            'https://www.consciousentities.com/'
    ]
    ...
    def parse1(self, response):
        urls = response.xpath('//aside[@id="archives-3"]/ul/li/a/@href').extract()
        if not urls:
            return
    ...
    def parse2(self, response):
        urls = response.xpath('//div[@id="content"]/article/header/h1/a/@href').extract()
        if not urls:
            return
        # txt = response.text.replace('\t', '').replace('\n', '')
        #
        ...
        for url in urls:
            ...
            # url = 'https://www.consciousentities.com/2017/04/the-hard-problem-of-physics/'
    ...
    def parse(self, response):
        ...
        item = OrderedDict()
        keys_texts = response.xpath('//footer[@class="entry-meta"]/a[@rel="tag"]/text()').extract()
        keywords = []
        ...
        contributor = response.xpath('//div[@class="entry-meta"]//span[@class="author vcard"]/a/text()').extract_first()
        if contributor:
