...
class babysittersinapinch_comSpider(Spider):
    name = "babysittersinapinch_com"
    count = 0
    custom_output = False
    use_selenium = True
    start_url = [
            'https://info.babysittersinapinch.com/blog/all'
    ]
    ...
    def parse1(self, response):
        urls = response.xpath('//div[@class="post-item"]/h2/a/@href').extract()
        if not urls:
            return
    ...
    def parse(self, response):
        ...
        title = response.xpath('//span[@id="hs_cos_wrapper_name"]/text()').extract_first().strip()
        # if not title:
        ...
        temp = response.xpath('//div[@id="hubspot-author_data"]/text()').extract()
        for t in temp:
            date_time_str = t.strip().replace('on ', '').replace(', ', ' ')
            if date_time_str:
                # date_time_str = 'Jun 28 2018 7:40AM'
                date_time_obj = datetime.datetime.strptime(date_time_str, '%b %d %Y %I:%M:%S %p')
        ...
        img_urls = response.xpath('//div[@class="section post-body"]//img')
        ...
        tags = response.xpath('//div[@class="section post-body"]/span[@id="hs_cos_wrapper_post_body"]/*')
        total_text = ''
