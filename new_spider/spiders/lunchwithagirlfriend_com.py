...
class lunchwithagirlfriend_comSpider(Spider):
    name = "lunchwithagirlfriend_com"
    count = 0
    custom_output = False
    use_selenium = True
    start_url = [
            'https://lunchwithagirlfriend.com/'
    ]
    ...
    def parse1(self, response):
        urls = response.xpath('//main[@class="content"]/article')
        if not urls:
            return
        for url_tag in urls:
            ...
            yield Request(url, self.parse2, meta={'img_url': img_url, 'img_alt': img_alt})
            # break

        cat_url = 'https://lunchwithagirlfriend.com/'
    ...
    def parse2(self, response):
        ...
        item = OrderedDict()
        keys_texts = response.xpath('//span[@class="entry-categories"]/a[@rel="category tag"]/text()').extract()
        keywords = []
        ...
        date = response.xpath('//time[@itemprop="datePublished"]/@datetime').extract_first()
        if date:
        ...
        main_img_tag = response.xpath('//div[@itemprop="text"]/figure//img')
        img_urls = response.xpath('//div[@itemprop="text"]/p//img')
        ...
        for i, tag in enumerate(tags):
            ...
            # else:
                ...
                text = delettags(text)
                if text == '<p></p>' or text == '<div></div>' or "uploads/2017/06/SapnaSig2.png" in text \
