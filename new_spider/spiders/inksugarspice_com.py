...
class inksugarspice_comSpider(Spider):
    name = "inksugarspice_com"
    count = 0
    custom_output = False
    use_selenium = True
    start_url = [
            'https://public-api.wordpress.com/wpcom/v2/sites/93440396/articles?className=is-style-default&showExcerpt=0&excerptLength=55&showReadMore=0&readMoreLabel=Keep reading&showDate=0&showImage=1&showCaption=0&imageShape=landscape&minHeight=9&moreButton=1&moreButtonText&showAuthor=0&showAvatar=1&showCategory=0&postLayout=grid&columns=2&postsToShow=10&mediaPosition=top&&&&&&typeScale=2&imageScale=3&mobileStack=0&sectionHeader&specificMode=0&textColor=black&customTextColor&singleMode=0&showSubtitle=0&postType%5B0%5D=post&wpcom_site=93440396&page=1'
    ]
    ...
    def start_requests(self):
        ...
        for url in self.start_url:
            yield Request(url, self.parse, meta={'page': 1, 'cat_url': url})

    def parse(self, response):
        json_data = json.loads(response.text)
        urls = response.xpath('//main[@id="main"]/div/article//h2[@class="entry-title"]/a/@href').extract()
        if not urls:
            return
        for url in urls:
            # url = 'https://www.skinnykitchen.com/recipes/4-skinny-stuffed-pepper-recipes-youll-love/'
            yield Request(url, self.parse2)
            # break

        cat_url = response.meta['cat_url']
        response.meta['page'] += 1
        yield Request(cat_url + 'page/' + str(response.meta['page']), self.parse, meta=response.meta)
    ...
    def parse2(self, response):
        ...
        item = OrderedDict()
        keys_texts = response.xpath('//span[@class="tag-links"]/a[@rel="tag"]/text()').extract()
        keywords = []
        ...
        contributor = response.xpath('//meta[@name="shareaholic:article_author_name"]/@content').extract_first()
        if contributor:
        ...
        main_img_tag = response.xpath('//div[@class="entry-content"]/figure//img')
        img_urls = response.xpath('//div[@class="entry-content"]/p//img')
        ...
        for i, tag in enumerate(tags):
            ...
            # else:
                ...
                text = delettags(text)
                if text == '<p></p>' or text == '<div></div>' or "sharedaddy sd-sharing-enabled" in text \
                        or 'id="jp-relatedposts"' in text or '<div style="display:none;"' in text \
