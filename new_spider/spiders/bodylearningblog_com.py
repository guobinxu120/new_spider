...
class bodylearningblog_comSpider(Spider):
    name = "bodylearningblog_com"
    count = 0
    custom_output = False
    use_selenium = True
    start_url = [
            'https://www.bodylearningblog.com/'
    ]
    ...
    def parse1(self, response):
        urls = response.xpath('//div[@class="wvrx-posts"]/article/header/h2/a/@href').extract()
        if not urls:
            return
        for url in urls:
    ...
    def parse(self, response):
        ...
        item = OrderedDict()
        keys_texts = response.xpath('//span[@class="tag-links"]/a/text()').extract()
        keywords = []
        for key in keys_texts:
            if len(key) > 50: continue
            keywords.append({'keyword': key.strip()})
        item['keywords'] = keywords
        title = response.xpath('//h1[@itemprop="headline name"]/text()').extract_first()
        # if not title:
        #     title = response.xpath('//td[@class="f2"]/text()').extract_first()
        item['title'] = title.strip()
        item['url'] = response.url
        contributor = response.xpath('//span[@itemprop="author"]/span[@itemprop="name"]/text()').extract_first()
        if contributor:
        ...
        img_urls = response.xpath('//div[@class="entry-content clearfix"]//img')
        ...
        item['images'] = images
        item['themes'] = [{"Id": 0, "ArticleId": 0, "Theme": "United State"}]
