...
class balanceguytraining_comSpider(Spider):
    name = "balanceguytraining_com"
    count = 0
    custom_output = False
    use_selenium = True
    start_url = [
            'http://balanceguytraining.com/blog-masonry/'
    ]
    ...
    def parse1(self, response):
        urls = response.xpath('//div[@class="mas-blog row"]/div[contains(@class,"mas-blog-post")]/div[@class="post-inner"]/h3/a/@href').extract()
        if not urls:
            return
        for url in urls:
        ...
        for i, tag in enumerate(tags):
            ...
            # else:
                ...
                if text == '<p></p>' or text == '<div></div>' or "sharedaddy sd-sharing-enabled" in text \
                        or 'entry-meta meta-tags' in text or 'lick Here To Buy Get Back In The Saddle' in text \
