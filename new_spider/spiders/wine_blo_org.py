...
class wine_blo_orgSpider(Spider):
    name = "wine_blo_org"
    count = 0
    custom_output = False
    use_selenium = True
    start_url = [
            'https://www.wine-blog.org/',
    ]
    ...
    def parse1(self, response):
        urls = response.xpath('//div[@id="content"]/div/p[@class="commentnum"]/a/@href').extract()
        if not urls:
            return
        for url in urls:
        ...
        next_url = response.xpath('//div[@class="alignright"]/a/@href').extract_first()
        if next_url:
            yield Request(next_url, self.parse1)

    def parse(self, response):
        post_id = response.xpath('//div[@id="content"]/div[@class="post"]/@id').extract_first().split('-')[-1]
        ...
        title = response.xpath('//h2[@class="title"]/a/text()').extract_first()
        # if not title:
        ...
        img_urls = response.xpath('//div[@class="entry"]//img')
        ...
        for img_tag in img_urls:
            ...
            if ('https://www.wine-blog.org' not in img_url) and ('http://www.wine-blog.org' not in img_url):
                continue
        ...
        tags = response.xpath('//div[@class="entry"]/*')
        total_text = ''
        for i, tag in enumerate(tags):
            ...
            # else:
                ...
                if text == '<p></p>' or text == '<div></div>' or 'sharedaddy sd-sharing-enabled' in text \
                        or 'post-tags' in text or 'scriptlesssocialsharing' in text \
